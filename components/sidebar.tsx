...
export function SidebarHeader({ className, ...props }: React.ComponentPropsWithoutRef<'div'>) {
  return (
    <div
      ...
      className={clsx(
        className,
        'flex flex-col border-b border-zinc-950/5 px-4 py-2 dark:border-white/5 [&>[data-slot=section]+[data-slot=section]]:mt-2.5'
      )}
    />
  )
}
...
/** Collapsible sidebar group (Calls, Messages, …). Opens when a child route is active. */
  ...
  return (
    <div className="flex flex-col gap-0.5">
      <Headless.Button
        ...
        className={clsx('cursor-pointer', sidebarItemClasses)}
      ...
      {open ? (
        <div className="ml-2.5 flex flex-col gap-0.5 border-l border-zinc-950/5 pl-2 dark:border-white/5">
          {children}
        </div>
      ) : null}
    </div>
